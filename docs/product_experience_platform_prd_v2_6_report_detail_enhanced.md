...
# 16. 实现期修正项与已知问题清单（V2.6.1 Patch Notes）

> 本节用于记录 V2.6 PRD 落地过程中已经完成的实现期修正，以及在落地过程中发现的、需要在后续版本继续跟进的问题。  
> 登记日期：2026-06-26  
> 来源：仓库提交历史（`git log`）与代码自查（`src/app/(main)/reports/[id]/page.tsx`、`src/components/issues/issue-rectification-dialog.tsx`、`src/lib/server/report-detail.ts`、`src/app/layout.tsx` 等）。

## 16.1 已落地修正项

### 16.1.1 报告详情视图压缩与排版收敛

| 编号 | 修正内容 | 影响范围 | 来源提交 |
|---|---|---|---|
| F-01 | 普通报告移除独立的"五感检查记录"区块，不合格项统一通过问题清单展示，避免重复 | Single Report Narrative Template | 28ddcb6 |
| F-02 | "功能效果"模块改为折叠卡片：默认显示食谱行 + 效果评分 + 问题数，点击展开步骤详情 | Single Report Narrative Template / 功能效果 Section | 28ddcb6 |
| F-03 | 型号合并报告顶部新增"问题点合并视图"，按阶段（手板/试制/试产/量产）→日期分段 | Model Dossier Timeline Template | 28ddcb6 |
| F-04 | 去除报告呈现中的 AI 字眼：AI 辅助结论 → 结论、AI 评分 → 评分、AI 总结 → 总结 | 全模板正文与打印 | 28ddcb6 |
| F-05 | 报告中心响应式布局改进，修复宽屏/窄屏切换时的栅格错位 | 报告中心首页 | 04a968b |
| F-06 | 报告媒体与打印布局稳定化，修护图文混排的分页断裂 | Print Block / Report Media | 63fda79 |
| F-07 | 打印图片 URL 规范化，统一相对/绝对路径转换 | Report Print Renderer | 5f21eb6 |
| F-08 | PDF 导出允许"仅有视频证据"以告警通过，不再硬阻断 | PDF 预检 / Profile | 9de056e |

### 16.1.2 整改与问题闭环

| 编号 | 修正内容 | 影响范围 | 来源提交 |
|---|---|---|---|
| R-01 | 抽出可复用 `IssueRectificationDialog`，报告页与问题管理页共用同一弹窗 | 报告详情 + 问题管理 | 28ddcb6 |
| R-02 | 问题点点击直接打开统一整改弹窗，支持"是否整改 / 整改方案 / 复测结果"记录 | 报告详情正文 | 28ddcb6 |
| R-03 | 问题状态更新服务端校验，拒绝非法状态跃迁 | Issues API | c5b82d3 |

### 16.1.3 移动端与录入体验

| 编号 | 修正内容 | 影响范围 | 来源提交 |
|---|---|---|---|
| M-01 | 修复移动端调用相机/录像：使用原生 `capture="environment"` 替代 `getUserMedia`，避免浏览器权限拒绝 | 录入/走查移动端 | 28ddcb6 |
| M-02 | 添加 viewport meta，修复移动端无排版/缩放异常 | 全站 Layout | 28ddcb6 |
| M-03 | 录入目录侧边栏支持纵向滚动，长目录不再被截断 | 录入页 | 28ddcb6 |
| M-04 | AI 辅助弹窗去除重复关闭按钮，避免误点 | AI 辅助弹窗 | 28ddcb6 |
| M-05 | 审计日志设置弹窗使用容器查询（container query）适配窄宽 | 审计日志设置 | 28ddcb6 |
| M-06 | 功能效果移动端动作布局稳定化 | 功能效果移动端 | 62f2c39 |
| M-07 | 五感证据动作布局稳定化 | 五感证据移动端 | 6f10b63 |

### 16.1.4 媒体、分享与权限

| 编号 | 修正内容 | 影响范围 | 来源提交 |
|---|---|---|---|
| P-01 | 报告媒体支持视频矩阵交付 | Report Media | 5f71617 / cff6eef |
| P-02 | 报告分享页交付稳定化，修复分享态下数据加载竞态 | Report Share | 1b291ae |
| P-03 | 登录表单值在受控/非受控切换中保持稳定 | 登录页 | 1eb8e87 |
| P-04 | 认证会话持久化安全修复，避免刷新后掉登录 | Auth Context | c747386 |
| P-05 | 允许审计用户清理自己的审计记录 | 审计日志 API | b693c20 |

## 16.2 落地过程中发现的问题清单（待跟进）

> 以下问题来自代码自查，尚未在 V2.6.1 中处理，需在 V2.6.2 或后续迭代中评估是否修正。

| 编号 | 问题描述 | 风险/影响 | 建议处理方式 |
|---|---|---|---|
| I-01 | 问题点合并视图 `PHASE_ORDER = ['手板','试制','试产','量产']` 为前端硬编码；若任务使用 `EVT/DVT/PVT`、`工程样机` 等阶段名，会被排到末尾甚至显示为"未指定阶段" | 跨项目阶段命名不统一时，型号合并报告阶段轴错乱 | 阶段字典服务端化，按项目类型配置阶段顺序；前端不再硬编码 |
| I-02 | 合并视图按 `phase + dateShort` 分组，同一阶段、同一日期但来自不同任务的报告会被并入同一组，组内无法区分任务来源 | 同日多任务合并时，问题来源追溯困难 | 分组键增加 `task_id`，或同组内按任务二次分块并显示任务标签 |
| I-03 | `findGroup` 用 `phase === phase && date === date` 匹配；当 `phase` 为 null 但日期相同的多个无阶段报告，会被错误合并为同一"日期组" | 无阶段报告间被隐式关联，可能误导为同批次 | 无阶段时不再按日期聚合，改为按报告独立展示，或显式标注"无阶段，按报告原序" |
| I-04 | `IssueRectificationDialog` 中 `STATUS_LIST = ['待整改','整改中','已验证','不整改']` 与 `LEVEL_LIST = ['一类','二类','三类']` 均为前端硬编码字符串；与 `schema.ts` 中枚举若发生漂移，会导致状态写入失败或回显失败 | 状态/等级字段向前端硬耦合，schema 演进时易出问题 | 状态/等级列表由服务端字典接口提供，前端只渲染 |
| I-05 | 普通报告移除独立"五感检查记录"区块后，五感合格项的逐项证据（照片/视频）在报告正文中不再有独立展示位，仅在问题清单中可见不合格项 | 合格项证据在报告中"消失"，审核时无法快速回看 | 在"五感/操作" Section 保留可折叠的合格项证据列表，默认收起，审核模式展开 |
| I-06 | 功能效果改为折叠卡片后，PDF/打印输出时是否自动展开步骤详情未在打印预检中明确约束 | 打印 PDF 可能缺少步骤详情，影响交付可读性 | 在 Print Block 规则中明确：功能效果 Section 在打印模板中强制展开步骤详情 |
| I-07 | 去 AI 化仅替换了呈现层措辞，数据库列名、API 字段（如 `ai_result`、`ai_summary`）仍保留 AI 命名 | 字段语义与呈现层不一致，未来对接外部系统时易引起误解 | 评估是否在 V2.7 中做字段重命名 + 兼容期，或保留字段名但在 API 文档中标注"呈现为结论/评分" |
| I-08 | `src/app/layout.tsx` 中 `viewport.maximumScale = 5` 在部分内嵌 WebView 中仍可能被忽略；移动端文字输入放大行为未覆盖 iOS Safari 边界 | 部分 WebView 中移动端排版仍异常 | 增加 `-webkit-text-size-adjust: 100%` 与字体 ≥16px on input 的兜底样式 |
| I-09 | 报告媒体视频矩阵交付后，PDF 导出对视频仍只输出缩略图 + 文件名，未包含可跳转的视频链接或二维码 | PDF 读者无法触达视频证据本体 | PDF 中为视频证据附短链/二维码，扫码或点击跳转分享页播放 |
| I-10 | 审计日志设置弹窗使用容器查询适配，但旧浏览器（无 `@container` 支持）会回退为默认布局，未做 polyfill | 兼容性边界 | 评估是否引入 container query polyfill 或对不支持环境降级为媒体查询布局 |

## 16.3 与 PRD 主体的关系

- 16.1 中的修正项是对 V2.6 章节内既定目标的实现期细化，不改变 V2.6 的对象、字段与权限模型。
- 16.2 中的问题清单是 V2.6.1 落地后发现的边界与缺口，建议在 V2.6.2 修订摘要中正式纳入"本版解决的核心问题"表，并对应到 V2.6 的 Section 5/6/9 章节。
- 本节不替代 V2.6 的 QA 验收方案（第 11 章），仅作为实现期记录与下一版输入。
