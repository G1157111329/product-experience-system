...
test('frozen issue reader keeps evidence at its source and exposes one dedicated status field', () => {
  ...
  assert.match(readerSource, /issue\.liveIssueId\s*&&\s*onManageIssue/, 'only a linked issue with management access may make the status interactive');
  assert.match(readerSource, /onClick=\{\(\) => onManageIssue\(issue\)\}/, 'the managed status itself opens rectification');
