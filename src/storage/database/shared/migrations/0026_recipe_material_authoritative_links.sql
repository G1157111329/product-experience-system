-- Keep recipe effect media reusable while making material_links authoritative.
-- This forward migration intentionally replaces the RPC instead of rewriting
-- the already-published 0017 migration and its journal hash.
...
BEGIN
  ...
  IF v_has_materials THEN
    ...
    FROM (
      ...
      FROM jsonb_array_elements_text(p_command->'material_ids') WITH ORDINALITY AS item(value, ordinality)
      GROUP BY value
