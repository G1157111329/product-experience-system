...
BEGIN
  ...
  IF v_has_materials THEN
    ...
    INTO v_material_ids
    FROM jsonb_array_elements_text(p_command->'material_ids');
        ...
        AND (
          ...
          OR EXISTS (
            ...
            WHERE ml.material_id = m.id
              AND NOT (ml.target_id = v_recipe_id AND ml.target_type IN ('recipe', 'recipe_effect'))
          )
        )
    ) THEN
  ...
  IF v_has_materials THEN
    UPDATE materials SET recipe_id = NULL
    WHERE recipe_id = v_recipe_id AND NOT (id = ANY(v_material_ids));
    UPDATE materials SET recipe_id = v_recipe_id WHERE id = ANY(v_material_ids);
  END IF;
  ...
  INTO v_materials FROM materials m WHERE m.recipe_id = v_recipe_id;
