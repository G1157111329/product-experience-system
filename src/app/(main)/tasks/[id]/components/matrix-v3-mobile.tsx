...
export function MatrixV3Mobile({ matrixId, taskId, projection, onChanged, attemptNavigation }: MatrixV3MobileProps) {
  ...
  const groups = useMemo(
    () => leaf
      ? buildMatrixMobileGroups({
          columns: projection.columns.filter((column) => column.columnZone !== 'primary_media'),
    ...
    <div className="space-y-3 md:hidden">
      <div className="flex items-center justify-between gap-2 text-xs text-muted-foreground">
        ...
        <div className="flex items-center gap-1">
          <Button type="button" size="sm" variant="outline" className="min-h-11 min-w-11 px-2" disabled={currentRowIndex === 0} onClick={() => void changeRow(-1)}><ChevronLeft className="mr-1 h-3.5 w-3.5" />上一行</Button>
          <Button type="button" size="sm" variant="outline" className="min-h-11 min-w-11 px-2" disabled={currentRowIndex === rows.length - 1} onClick={() => void changeRow(1)}>下一行<ChevronRight className="ml-1 h-3.5 w-3.5" /></Button>
        </div>
      </div>
      {leaf && (() => {
              ...
              {groups.map((group) => {
                ...
                return (
                  <section key={group.id} className="rounded-md border">
                    <button type="button" className="flex min-h-11 w-full items-center gap-2 px-2.5 py-2 text-left text-xs font-medium" onClick={() => setExpandedGroups((state) => ({ ...state, [groupKey]: !open }))}>
                ...
                {group.columns.map((col) => {
                  ...
                  if (col.columnZone === 'calculation_dimension') {
                    const display = cell?.valueNumber != null
                      ? formatMatrixNumber(cell.valueNumber, col.decimalPlaces)
                      : cell?.displayText ?? '—';
                    return <div key={col.id} className="space-y-1"><p className="text-xs font-medium text-muted-foreground">{col.columnLabel}</p><p className="rounded bg-muted px-2 py-1.5 text-sm font-mono">{display}</p></div>;
                  }
