...
/**
 * MatrixMaterialStagingRail — task-scoped staging pool (PRD §9.2).
 * Lists materials from GET /api/v1/materials/staging?taskId= for quick bind.
 */
...
interface MatrixMaterialStagingRailProps {
  taskId: string;
  className?: string;
  /** Optional: drag payload helper — parent can listen for dragstart. */
...
export function MatrixMaterialStagingRail({
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/v1/materials/staging?taskId=${encodeURIComponent(taskId)}`, {
        cache: 'no-store',
      });
      ...
      if (json.code === 0) {
        ...
        toast.error(json.message || '加载暂存池失败');
      }
    } catch {
      toast.error('加载暂存池失败');
    } finally {
    ...
    <div className={cn('rounded-lg border bg-card flex flex-col min-h-0', className)}>
      <div className="flex items-center justify-between gap-2 px-3 py-2 border-b">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-sm font-medium truncate">素材暂存池</span>
        ...
        <div className="p-2 grid grid-cols-4 sm:grid-cols-5 lg:grid-cols-3 gap-1.5">
          {loading && items.length === 0 ? (
            ...
            <p className="col-span-full text-xs text-muted-foreground py-6 text-center px-2">
              暂无待归属素材。上传到任务后会出现在此，可拖到矩阵 D/O 槽或在选择器中选用。
            </p>
          ) : (
            items.map((m) => (
              <button
