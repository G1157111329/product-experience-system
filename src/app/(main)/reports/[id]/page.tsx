                    ...
                    return (
                      <div className="space-y-1.5 ml-5">
                        <div className="grid grid-cols-4 gap-1">
                          {dims.map(d => (
                            <div key={d.label} className="flex items-center gap-1 p-1 rounded bg-background/80">
                              ...
                              <span className={`text-[10px] font-bold px-1 py-0.5 rounded-full ${getScoreColor(d.data?.score || 0)}`}>
                                {d.data?.score || 0}
                              </span>
                            </div>
                          ))}
                        ...
                        {dims.map(d => d.data?.comment && (
                          <div key={`${d.label}-cmt`}>
                            <span className="text-[10px] font-medium">{d.icon} {d.label}：</span>
                            <span className="text-[10px] text-muted-foreground">{d.data.comment}</span>
                          </div>
                        ))}
                        {ai.overall?.summary && (
                          <div className="pt-1 border-t border-border/50">
                            <span className="text-[10px] font-medium">📋 综合：</span>
                            <span className="text-[10px] text-muted-foreground">{ai.overall.summary}</span>
                          </div>
                        )}
                      </div>
                    );
