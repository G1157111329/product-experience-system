...
export default function DashboardPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      else setData({ totalTasks: 0, completedTasks: 0, totalIssues: 0, resolvedIssues: 0, recentTasks: [], recentIssues: [] });
    } catch {
      setData({ totalTasks: 0, completedTasks: 0, totalIssues: 0, resolvedIssues: 0, recentTasks: [], recentIssues: [] });
  ...
  const stats = [
    ...
    { label: '完成率', value: `${completionRate}%`, icon: CheckCircle2, color: 'text-primary', bg: 'bg-primary/10' },
    { label: '问题总数', value: data.totalIssues ?? 0, icon: AlertTriangle, color: 'text-amber-600', bg: 'bg-amber-50' },
    { label: '整改率', value: `${resolutionRate}%`, icon: Clock, color: 'text-orange-700', bg: 'bg-orange-100' },
  ];
