...
export function IssueRectificationDialog({ issue, open, onOpenChange, onSaved }: IssueRectificationDialogProps) {
  ...
  useEffect(() => {
    if (open && current?.id && current.source_type === 'recipe_problem') {
  ...
  }, [open, current?.id, current?.source_type, fetchReEvaluations]);
  ...
  const updateImproveFlag = async (willImprove: boolean) => {
    if (!current) return;
    const nextStatus = willImprove ? (current.status === '不整改' ? '待整改' : current.status) : '不整改';
    try {
      const res = await fetch(`/api/issues/${current.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ is_improve: willImprove, status: nextStatus }),
      });
      const data = await res.json();
      if (data.code === 0) {
        const next = { ...current, is_improve: willImprove, status: nextStatus };
        setCurrent(next);
        onSaved?.(next);
      } else {
        toast.error(data.message || '保存失败');
  ...
  const willImprove = current?.is_improve !== false && current?.status !== '不整改';
                ...
                <div className="grid grid-cols-4 gap-1">
                  {STATUS_LIST.map((s) => (
                    <button
                      ...
                      className={cn(
                        'min-h-9 whitespace-nowrap rounded border px-2 py-1.5 text-xs font-medium transition-colors',
                        current.status === s ? STATUS_COLORS[s] + ' border-current' : 'bg-background border-border hover:bg-muted/50'
                      )}
                    >
                      {s}
                    </button>
                ...
                <div className="grid grid-cols-2 gap-1">
                  <button
                    ...
                    className={cn(
                      ...
                      willImprove ? 'bg-primary text-primary-foreground border-current' : 'bg-background border-border hover:bg-muted/50'
                    )}
                  >
                    整改
                  </button>
