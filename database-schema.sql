...
CREATE TABLE IF NOT EXISTS issue_re_evaluations (
  ...
  description TEXT,
  result VARCHAR(20) NOT NULL DEFAULT 'pending' CHECK (result IN ('qualified', 'unqualified', 'pending')),
...
CREATE TABLE IF NOT EXISTS recipes (
  ...
  effect_status VARCHAR(20) NOT NULL DEFAULT 'pending' CHECK (effect_status IN ('qualified', 'unqualified', 'pending')),
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ DEFAULT now()
);
...
-- ---- 0016: canonical evaluation status + atomic issue retest ----
...
DO $$
BEGIN
  IF to_regclass('public.issue_status_dict') IS NOT NULL THEN
...
FROM (
  SELECT id,
    FIRST_VALUE(id) OVER (
      PARTITION BY source_type, CASE WHEN source_type = 'recipe_problem' THEN recipe_id ELSE record_id END
  ...
  FROM issues
  WHERE source_report_id IS NULL AND (
...
UPDATE materials target SET issue_id = mapping.keeper_id FROM _issue_merge_0016 mapping WHERE target.issue_id = mapping.duplicate_id;
UPDATE issue_re_evaluations target SET issue_id = mapping.keeper_id FROM _issue_merge_0016 mapping WHERE target.issue_id = mapping.duplicate_id;
DO $$
BEGIN
  IF to_regclass('public.issue_occurrences') IS NOT NULL THEN
    EXECUTE 'UPDATE issue_occurrences target SET issue_id = mapping.keeper_id FROM _issue_merge_0016 mapping WHERE target.issue_id = mapping.duplicate_id';
  END IF;
  IF to_regclass('public.rectification_actions') IS NOT NULL THEN
    EXECUTE 'UPDATE rectification_actions target SET issue_id = mapping.keeper_id FROM _issue_merge_0016 mapping WHERE target.issue_id = mapping.duplicate_id';
  END IF;
  IF to_regclass('public.verifications') IS NOT NULL THEN
    EXECUTE 'UPDATE verifications target SET issue_id = mapping.keeper_id FROM _issue_merge_0016 mapping WHERE target.issue_id = mapping.duplicate_id';
  END IF;
  IF to_regclass('public.matrix_issue_points') IS NOT NULL THEN
    EXECUTE 'UPDATE matrix_issue_points target SET linked_issue_id = mapping.keeper_id FROM _issue_merge_0016 mapping WHERE target.linked_issue_id = mapping.duplicate_id';
  END IF;
END;
$$;
...
DECLARE v_status TEXT; v_subject TEXT;
BEGIN
  ...
  ELSE
    ...
    VALUES (NEW.task_id, NEW.id, evaluation_issue_title(NEW.check_item, 'record', v_status), COALESCE(NEW.problem_level, '二类'),
...
CREATE TRIGGER recipes_evaluation_issue_sync AFTER INSERT OR UPDATE OF effect_status, name, recipe_type ON recipes
...
ALTER TABLE recipes ADD CONSTRAINT recipes_effect_status_check CHECK (effect_status IN ('qualified', 'unqualified', 'pending'));
...
ALTER TABLE issue_re_evaluations ADD CONSTRAINT issue_re_evaluations_result_check CHECK (result IN ('qualified', 'unqualified', 'pending'));
...
SELECT r.task_id, r.id,
  evaluation_issue_title(CASE WHEN right(trim(r.name), 2) IN ('食谱', '功能') THEN trim(r.name)
    ...
    'recipe', r.effect_status), '二类', '功能/食谱效果评价', 'recipe_problem', 'open'
FROM recipes r WHERE normalize_evaluation_status(r.effect_status) IN ('pending', 'unqualified')
ON CONFLICT (recipe_id) WHERE source_type = 'recipe_problem' AND recipe_id IS NOT NULL AND source_report_id IS NULL
DO UPDATE SET title = EXCLUDED.title, updated_at = now();
INSERT INTO issues (task_id, record_id, title, level, source, source_type, description, status)
SELECT r.task_id, r.id, evaluation_issue_title(r.check_item, 'record', r.evaluation_result), COALESCE(r.problem_level, '二类'),
  CASE WHEN r.standard_category IS NULL THEN '体验检查' ELSE r.standard_category || '问题' END,
  'record_fail', r.problem_description, 'open'
FROM check_records r WHERE normalize_evaluation_status(r.evaluation_result) IN ('pending', 'unqualified')
ON CONFLICT (record_id) WHERE source_type = 'record_fail' AND record_id IS NOT NULL AND source_report_id IS NULL
DO UPDATE SET title = EXCLUDED.title, description = EXCLUDED.description,
  level = EXCLUDED.level, source = EXCLUDED.source, updated_at = now();
...
BEGIN
  IF v_action NOT IN ('create', 'update', 'delete') THEN RAISE EXCEPTION 'unsupported retest action'; END IF;
  IF v_action IN ('create', 'update') AND p_command ? 'result' AND v_result NOT IN ('qualified', 'unqualified', 'pending') THEN RAISE EXCEPTION 'invalid retest result'; END IF;
  ...
  ELSE
    SELECT issue_id INTO v_issue_id FROM issue_re_evaluations WHERE id = v_retest_id FOR UPDATE;
  ...
  IF v_action IN ('create', 'update') AND p_command ? 'material_ids' THEN
    ...
    SELECT count(*) INTO v_valid_material_count FROM materials material
    WHERE material.id IN (SELECT DISTINCT value FROM jsonb_array_elements_text(p_command->'material_ids'))
    ...
    UPDATE materials SET re_evaluation_id = NULL WHERE re_evaluation_id = v_retest_id;
    UPDATE materials SET re_evaluation_id = v_retest_id
    WHERE id IN (SELECT jsonb_array_elements_text(COALESCE(p_command->'material_ids', '[]'::jsonb)));
  END IF;
  SELECT result INTO v_latest_result FROM issue_re_evaluations WHERE issue_id = v_issue_id ORDER BY created_at DESC, id DESC LIMIT 1;
  UPDATE issues SET status = CASE v_latest_result
    WHEN 'qualified' THEN 'verified_closed' WHEN 'unqualified' THEN 'rectifying'
    WHEN 'pending' THEN 'open' ELSE 'open' END,
    updated_at = now() WHERE id = v_issue_id;
  SELECT to_jsonb(row_value) INTO v_retest_json FROM issue_re_evaluations row_value WHERE id = v_retest_id;
  SELECT to_jsonb(row_value) INTO v_issue_json FROM issues row_value WHERE id = v_issue_id;
...
BEGIN
  ...
  IF p_command ? 'effect_status' THEN
    v_status := NULLIF(trim(COALESCE(p_command->>'effect_status', '')), '');
  END IF;
  IF v_status NOT IN ('qualified', 'unqualified', 'pending') THEN
    RAISE EXCEPTION 'invalid evaluation status';
  END IF;
  IF p_command ? 'effect_description' THEN
    v_description := p_command->>'effect_description';
  END IF;
  ...
  UPDATE recipes
  SET effect_status = v_status,
      effect_description = v_description,
      name = v_name,
      ingredients = v_ingredients,
      recipe_type = v_recipe_type,
      problem_count = v_problem_count,
      ingredient_items = v_ingredient_items,
      updated_at = NOW()
  WHERE id = v_recipe_id;
  GET DIAGNOSTICS v_updated_count = ROW_COUNT;
  IF v_updated_count <> 1 THEN
    RAISE EXCEPTION 'recipe update affected zero rows';
  END IF;
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'service_role') THEN
    ...
    GRANT EXECUTE ON FUNCTION public.apply_issue_retest(JSONB) TO service_role;
  END IF;
END;
$$;

-- 0025_frozen_media_reference_guard: keep immutable report media and audit
-- storage cleanup failures after a successful database deletion.
...
-- Upgrade pre-existing cleanup queues; CREATE TABLE IF NOT EXISTS alone does
-- not add the retention worker columns required by the startup manifest.
...
INSERT INTO frozen_material_references(snapshot_id, material_id)
SELECT snapshot.id, material.id
FROM report_snapshots snapshot
JOIN materials material ON snapshot.snapshot_json::text LIKE ('%' || material.id || '%')
ON CONFLICT DO NOTHING;
...
CREATE TRIGGER materials_frozen_delete_guard BEFORE DELETE ON materials
