...
- 报告录入页的 AI 总结不得生成、保存或展示满意度、评分、得分、分数、分级或 `/10` 等评分事项；即使管理员保留了旧自定义 Prompt，服务端硬性输出约束和解析清洗仍须覆盖。总结编辑框按冻结页层级呈现：总结正文分段，主要优势/主要风险/后续建议逐条列出，保存时兼容旧的“标题：内容”格式。
- 冻结报告的食谱/功能 Tab 中，效果素材与步骤素材使用紧凑缩略图；右侧结果标签固定为合格绿色、不合格红色、待定灰色。浏览器打印页和服务端 PDF 必须复用相同状态颜色与紧凑素材尺寸。
