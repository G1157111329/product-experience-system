...
- **冻结报告、分享与打印前端版式（2026-07-17）**：
  - 冻结报告阅读器 `src/components/reports/frozen-report-reader.tsx`：Tab 栏使用 `ReportTabBar` 并包裹在 sticky 容器内（`top-[68px]`），保持模块导航锚定；统一 tab-panel 内边距为 `px-4 py-5 sm:px-6 sm:py-6 lg:px-8 lg:py-7`；问题卡片改为 `rounded-xl border bg-background p-4 shadow-sm sm:p-5`，标题行 `flex items-start gap-2 sm:items-center`，问题描述 `text-[15px] font-semibold leading-relaxed`，状态标签为 `h-7 rounded-full border px-2.5 text-xs`；展开详情区统一为 `mt-4 space-y-5 border-t pt-4`，整改素材、复测证据按分组展示；功能效果卡片改为 `rounded-xl border bg-background shadow-sm`，header 区使用 `bg-muted/40 px-4 py-3` 并在大屏下 `sm:flex-row sm:items-center sm:justify-between`，步骤 `details` 使用 `rounded-lg border bg-muted/10 px-3 py-2 open:bg-muted/20`。
  - 分享页 `src/app/reports/share/[token]/page.tsx`：整体容器为 `min-h-screen bg-slate-100 px-3 py-4 sm:px-6 sm:py-6`，卡片 `mx-auto max-w-[1180px] rounded-xl bg-white shadow-sm sm:rounded-2xl`；header 移动端纵向排列、桌面端横向排列，标题区使用 `min-w-0` 避免下载按钮挤压；内容区 `px-4 py-5 sm:px-6 sm:py-7`；页脚显示“产品体验管理平台 · 分享报告（仅查看） · {shareDate}”。
  - 打印页 `src/app/reports/print/page.tsx`：屏幕预览模式使用 `min-h-screen bg-slate-100 print:bg-white`；顶部固定 `print:hidden` 预览工具栏，左侧显示报告标题、右侧“打印 / 保存 PDF”按钮；文档区域 `mx-auto max-w-[1180px] bg-white p-4 shadow-sm sm:p-6 print:shadow-none`，保留 `@media print` 白底与 `print-text-mode` 隐藏图片逻辑。
  - 媒体网格 `src/components/reports/report-media-grid.tsx`：`primary` 预览 `minWidth: 120, maxWidth: 160`；`appendix` 与 `compact` 统一为 `72×72`，`appendix` 显示上限提升到 8、`compact` 上限提升到 3；`share-paper-compact` 最小 34px、最大 38px。
  - 打印/PDF 渲染器 `src/components/reports/report-section-block-renderer.tsx`：`PaperMedia` 支持三种密度：`standard`（96×72，用于功能效果/食谱步骤证据）、`comfortable`（72×54，用于问题点证据、整改素材、复测证据）、`compact`（34×26，用于矩阵）。`PrintMediaThumbs` 静态缩略图由 58×46 提升到 72×54。矩阵（comparison / data_matrix / data_matrix_v3）保持既有表格版式不变。
...
- 食谱/功能效果以及问题展开区的原位素材使用紧凑证据尺寸，图片/视频缩略图约 64-80px；视频与同角色图片保持相同 `4:3` 外框，并显示底部 `VIDEO` 标识。不得用占满内容栏的大图替代证据缩略图。
...
- 效果素材必须在原单元格内全部呈现，使用从左到右的自适应缩略图网格：桌面约 48-52px，窄容器可降至约 32px；图片与视频均保持 `4:3` 外框，视频显示 `VIDEO` 标识。不得显示 `+N`、单列纵向素材墙或越出单元格。
...
- **匿名分享阅读合同**：`/reports/share/[token]` 使用冻结打印模型与 `ReportPrintDocument` 的阅读版式，不恢复平台 Tab/操作布局；分享页素材必须继续经过分享令牌预签名，支持图片原图放大和视频播放。顶部左侧固定加粗“产品体验管理平台 · 匿名只读分享”，右侧保留下载按钮。分享页下载必须等待当前页图片完成或最多 5 秒后直接调用 `window.print()`，打开系统打印/另存为对话框；不得新开 `/reports/print` 或把用户停留在单独的打印页面，下载按钮不得进入 PDF。
...
- **视频放行与企业网络合同（2026-07-17）**：视频海报必须从已解析的受保护视频地址派生，保留相同 `token` 与 `exp`，分享、冻结与素材库不得以原始 key 请求无签名 `/api/materials/poster/*`。冻结页、匿名分享和浏览器打印的同角色视频缩略图均使用与图片一致的 `4:3` 外框和底部 `VIDEO` 标识。若浏览器将同源 `/api/materials/video/<base64url-key>` 重定向到 `172.* /disable/disable.htm`，这是企业网关“禁用下载/网盘”策略；后续不得通过恢复公开 `/uploads`、移除签名、伪装媒体或修改 CORS 绕过。应保留 Network 证据并由网络管理员放行平台主机的 `/api/materials/video/*`、`/api/materials/poster/*` 与 `video/mp4` 响应，且禁止重定向至拦截页；重定向后的 CORS/PNA 报错是次生现象。
...
- **V3.1.2.4 AI助手大版本基线（2026-07-17）**：平台内嵌 AI 助手运行时、对话持久化/SSE、结构化操作计划、用户确认执行、操作审计及任务/记录/食谱/矩阵/素材低中风险动作已交付；生产开启 `hermes_agent_gateway_enabled` 后才可使用。对外公告统一称“AI助手”，不得宣称 Hermes 是独立服务，也不得把尚未通过官方凭证与回调验收的企微素材接入作为已上线能力。
...
### 2026-07-21 整改状态一致性合同

- 每个问题的整改状态和当前整改方案以实时 `issues` 记录为唯一可变真源。报告事实保持冻结，但冻结详情、浏览器下载/打印、PDF 与问题管理必须按同一问题身份叠加同一实时状态与当前方案。
- 可选状态固定为 `open`（待整改）、`rectifying`（整改中）、`verified_closed`（整改完成）、`waived`（不整改）。有权限用户可直接选择任一状态，禁止以前序状态限制或禁用选项；直接标记整改完成仍必须原子创建可审计的整改动作和验证记录。
- 保存整改字段或状态后必须采用服务端返回的主问题记录、明显确认当前已保存状态，并刷新冻结报告投影而不改变当前报告 Tab。打印/PDF 必须优先使用 `issues.improve_plan`，不得以过期的 `rectification_actions.action_plan` 覆盖当前整改方案。
