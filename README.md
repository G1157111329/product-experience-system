...
- 生产下载/打印页面对受保护素材始终通过报告范围预签名读取，`mode=fast` 不允许请求裸 `/uploads/*`，也不会等待图片下载或 Canvas 转码；网络请求 10 秒超时，单个失败素材不阻塞整份报告。冻结食谱/功能素材使用紧凑缩略图，结果标签为合格绿色、不合格红色、待定灰色，并与 PDF 保持一致。
