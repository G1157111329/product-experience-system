...
- 报告录入的 AI 总结固定采用“总结正文 + 主要优势/主要风险/后续建议”分段条目格式；服务端会忽略自定义提示词或历史数据中的满意度、评分、得分、分数和 `/10` 等评分事项。
- 生产下载/打印页面对受保护素材始终通过报告范围预签名读取，`mode=fast` 不允许请求裸 `/uploads/*`。冻结食谱/功能素材使用紧凑缩略图，结果标签为合格绿色、不合格红色、待定灰色，并与 PDF 保持一致。
