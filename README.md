...
| POST | `/api/recipes/[id]/ai-detect-problems` | AI问题点识别（第一层：负面情绪语言；第二层：专业产品评价官视角，期待vs实际体验差距） |
...
删除五感体验条目、食谱步骤、食谱等条目时，仅解除素材关联（将关联字段设为null），不会删除素材本身。素材仍保留在素材仓库中。DB外键已从 `onDelete("cascade")` 改为 `onDelete("set null")`。
