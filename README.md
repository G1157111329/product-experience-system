...
- 完成五感体验、食谱/功能、对比矩阵、动态数据矩阵、统一问题整改与冻结报告/分享/打印/PDF 的全链路升级。
- 新增平台内嵌 **AI助手**：复用平台模型配置，支持对话持久化、结构化操作计划、用户确认后执行任务/记录/食谱/矩阵/素材等低中风险业务动作，并保留审计记录。
- 生产已启用 AI 助手网关；企微素材接入仍须完成官方凭证与回调验收后才可启用，不能作为当前已交付能力宣传。
